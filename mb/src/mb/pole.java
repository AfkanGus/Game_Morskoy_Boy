...
public class pole extends JPanel
{
	...
	public class myMouse1 implements MouseListener {
		...
		public void mousePressed(MouseEvent e) {
			// Если сделано одиночное нажатие левой клавиши мыши
			if ((e.getButton() == 1) && (e.getClickCount() == 1)) {
				// Получаем текущее координаты курсора мыши
				mX = e.getX();
				mY = e.getY();
	...
	public pole()
	{
		// Подключаем обработчик события для мыши к панели
		addMouseListener(new myMouse1());
		addMouseMotionListener(new myMouse2());
		
		setFocusable(true); // Передаем фокус панели
		
		// Создадим объект новой игры
		myGame = new game();
		// Запускаем игру
		myGame.start();
		...
		try
		{
			...
			paluba = ImageIO.read(new File("c:\\mb\\paluba.png"));
			ranen = ImageIO.read(new File("c:\\mb\\ranen.png"));
			ubit = ImageIO.read(new File("c:\\mb\\ubit.png"));
			end1 = ImageIO.read(new File("c:\\mb\\end1.png"));
			end2 = ImageIO.read(new File("c:\\mb\\end2.png"));
			bomba = ImageIO.read(new File("c:\\mb\\bomba.png"));
		}
		catch(Exception ex) {}
		...
		// Включаем возможность произвольного размещении элементов интерфейса на панели
		setLayout(null);
	...
	public void paintComponent(Graphics gr)
	{
		...
		// Отрисовка игровых полей Компьютера и Игрока на основе массивов
		for (int i=0; i<10; i++) {
			for (int j=0; j<10; j++) {
		...
		// Выведение цифр и букв слева и сверху от игровых полей
		for (int i=1; i<=10; i++)
		{
			// Вывод цифр
		...
		{
			gr.drawImage(end1, 300, 200, 300, 200, null);
		}
		else if (myGame.endg == 2) // Если победил Компьютер
		{
			gr.drawImage(end2, 300, 200, 300, 200, null);
		}
	}
}