...
public class game {
	...
	public int[][] masComp;
	// Признаки хода компьютера (false - ходит игрок)
	public boolean compHod;
	// Признаки конца игры (0-игра идет, 1-победил игрок, 2-победил компьютер)
	public int endg;
	
	// Конструктор класса
	public game() {
		// Создаем массив 10х10 - игровое поле игрока
		masPlay = new int[10][10];
		// Создаем массив 10х10 - игровое поле компьютера
		masComp = new int[10][10];
	}
	
	// Запуск игры - начало игры
	public void start() {
		// Очищаем игровое поле игрока компьютера
		for (int i=0; i<10; i++) {
			for (int j=0; j<10; j++) {
				masPlay[i][j] = 0;
				masComp[i][j] = 0;
	...
	private void analizUbit(int[][] mas, int i, int j, int kolPalub)
	{
		...
		int kolRanen = 0;
		// Выполнить подсчет раненых палуб
		...
		// Если количество раненых палуб совпадает с количеством палуб корабля, то он убит - прибавляем число 7
	...
	// Выстрел компьютера - возвращает истину, если попал
	private boolean compHodit()
	{
		...
		// Цикл for-each - счетчик в такой конструкции не применяется
	...
	// Установить одни элемент окружения подбитого корабля
	private void setOkrPodbit(int[][] mas, int i, int j) {
		// Если не происходит выход за пределы массива и в ячейке нулевое значение
	...
	{
		// Тестовое число = 15*4+16*2*3+17*3*2+18*4 - ситуация, когда все корабли убиты
		int testNumber = 330;
	...
	private void setMasValue(int[][] mas, int i, int j, int val) {
		// Если не происходит выхода за границы массива
		if (testMasPoz(i, j) == true) {
	...
	// Установим один элемент окружения
	private void setOkr(int[][] mas, int i, int j, int val) {
		// Если не происходит выхода за пределы массива и в ячейке нулевое значение
	...
	private void make1P(int[][] mas) {
		...
		for (int k=1; k<=4; k++) {
			// Глухой цикл while
			while (true) {
				// Находим случайную позиции на игровом поле
				int i = (int)(Math.random()*10);
				int j = (int)(Math.random()*10);
	...
	private boolean testNewPaluba(int[][] mas, int i, int j) {
		// Если есть выход за границы массива
